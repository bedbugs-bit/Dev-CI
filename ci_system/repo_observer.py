...
def poll():
    ...
    while True:
        try:
            ...
            raise Exception("Repository update failed: " + e.output.decode())
        ...
        if commit_id_file.is_file():
            try:
                ...
                raise Exception("Could not contact dispatcher: " + str(e))
            if response == "OK":
                ...
                response = helpers.communicate(dispatcher_host, dispatcher_port, dispatch_message)
                if response != "OK":
